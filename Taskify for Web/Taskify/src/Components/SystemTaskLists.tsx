...
class SystemTaskLists extends React.Component<TaskListProps> {
  handleTaskListSelected(id: string) {
    // eslint-disable-next-line no-shadow
    const { setSelectedTaskList } = this.props;

    if (setSelectedTaskList != null) {
      setSelectedTaskList(new SelectedListStateData(id, false));
    }
  }
        ...
        {taskLists?.map((task, _index) => {
          return (
            <ListItem
              button
              key={task.name}
              onClick={() => this.handleTaskListSelected(task.id)}
            >
...
export default connect(mapStateToProps, mapDispatchToProps)(SystemTaskLists);
