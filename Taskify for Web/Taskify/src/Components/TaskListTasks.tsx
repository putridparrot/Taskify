    ...
    return (
      <List> {
        taskList?.tasks.map((task, _index) => {
          return (
            <Box borderBottom={1}>
              <ListItem button key={task.text} onClick={()=>this.handleTaskSelected(task)}>
                {/* <ListItemIcon>
...
function getTaskList(state: any) {
  ...
  if (selectedTaskList != null) {
    if (selectedTaskList.isUser) {
      return ArrayExtensions.firstOrDefault(state.userTaskLists, tl => tl.id === selectedTaskList.id);
    } if (selectedTaskList.isSystem){
      return ArrayExtensions.firstOrDefault(state.systemTaskLists, tl => tl.id === selectedTaskList.id);
    }
  }
  return null;
}
